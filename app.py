...
<style>
    ...
    div[data-testid="stTextInput"] input { border: 4px solid #0056b3 !important; border-radius: 15px; padding: 25px !important; font-size: 28px !important; font-weight: 800; }
    div[data-testid="stFormSubmitButton"] button { width: 100%; background-color: #0056b3 !important; color: white !important; font-weight: 900; border-radius: 15px; padding: 20px; border: none; font-size: 30px; transition: 0.3s; }
    ...
    /* 주소 카드 */
    .address-card { background: white; padding: 35px; border-radius: 20px; border-left: 15px solid #0056b3; margin-bottom: 35px; box-shadow: 0 10px 20px rgba(0,0,0,0.08); }
    ...
    .metric-card { flex: 1; background: white; padding: 30px 10px; border-radius: 18px; border: 2px solid #e9ecef; text-align: center; box-shadow: 0 6px 12px rgba(0,0,0,0.05); }
    ...
    .info-box { flex: 1; background: #e9ecef; padding: 22px; border-radius: 15px; text-align: center; border: 2px solid #ced4da; }
...
def search_building(query_str):
    ...
    found = []
    # 에러 방지: 데이터에 있는 칼럼만 안전하게 불러오기
    for chunk in pd.read_csv(f_master, dtype=str, chunksize=70000, usecols=lambda x: clean_txt(x) in cols):
...
def load_all_details(pks):
    ...
    if os.path.exists("suwon_floor_info.csv.gz"):
        ...
        # 에러 방지: 구조코드명이 없어도 튕기지 않게 람다 적용
        for chunk in pd.read_csv("suwon_floor_info.csv.gz", dtype=str, chunksize=70000, usecols=lambda x: clean_txt(x) in f_cols):
            chunk.columns = [clean_txt(c) for c in chunk.columns]
    ...
    if os.path.exists("suwon_unit_status.csv.gz") and os.path.exists("suwon_unit_area.csv.gz"):
        for chunk in pd.read_csv("suwon_unit_status.csv.gz", dtype=str, chunksize=70000, usecols=lambda x: clean_txt(x) in ['관리건축물대장PK', '호명칭', '층번호']):
            chunk.columns = [clean_txt(c) for c in chunk.columns]
            res = chunk[chunk['관리건축물대장PK'].isin(pks)]
            if not res.empty: s_list.extend(res.to_dict('records'))
            
        for chunk in pd.read_csv("suwon_unit_area.csv.gz", dtype=str, chunksize=70000, usecols=lambda x: clean_txt(x) in ['관리건축물대장PK', '호명칭', '층번호', '전유공용구분코드', '면적(㎡)']):
            chunk.columns = [clean_txt(c) for c in chunk.columns]
...
if submitted and query:
    ...
    if items:
            ...
            if "집합" in str(b.get('대장구분코드명', '')):
                ...
                if my_f:
                    ...
                    for _, row in f_df.iterrows():
                        ...
                        # 파일에 '구조코드명'이 있을 때만 표출하도록 안전 처리
                        struct_info = f" ({row.get('구조코드명')})" if '구조코드명' in row and pd.notna(row['구조코드명']) else ""
                        
                        tbl += f'<tr><td class="row-floor">{row.get("층번호")}층</td><td>{row.get("주용도코드명", "-")}{struct_info}</td><td style="text-align:center; font-weight:800;">{u_info}</td><td style="text-align:right; font-weight:900; color:#d9480f;">{row.get("면적(㎡)", "-")} ㎡</td></tr>'
                    tbl += '</table>'
                    st.markdown(tbl, unsafe_allow_html=True)
        ...
        st.error("입력하신 지번을 찾을 수 없습니다. (망포동 6-11 등 정확한 지번을 입력해주세요)")
