...
<style>
    ...
    .bld-header {
        font-size: 20px; font-weight: 800; color: #007bff; 
        margin-top: 30px; margin-bottom: 10px; padding-bottom: 5px; 
...
def load_data():
    # 건물명과 동명칭을 추가로 가져와 여러 건물 구분
    master_cols = ['대지위치', '관리건축물대장PK', '대장구분코드명', '주용도코드명', '건물명', '동명칭', '위반건축물여부', '지상층수', '가구수(가구)', '세대수(세대)', '사용승인일', '옥내자주식대수(대)', '옥외자주식대수(대)', '승용승강기수', '비상용승강기수']
...
if submitted:
    if not query:
        ...
        if not res.empty:
            st.success(f"✅ 해당 지번에서 총 {len(res)}개의 건축물이 확인되었습니다.")
            
            # 검색된 모든 건축물(주택, 상가, 기숙사 등)을 반복해서 모두 출력
            for idx, item in res.iterrows():
                pk = str(item.get('관리건축물대장PK', ''))
                bld_name = str(item.get('건물명', '')).strip()
                dong_name = str(item.get('동명칭', '')).strip()
                
                # 건물명이나 동명칭이 있으면 표시, 없으면 '건축물 1', '건축물 2'로 구분
                name_display = []
                if bld_name and bld_name != 'nan': name_display.append(bld_name)
                if dong_name and dong_name != 'nan': name_display.append(dong_name)
                name_str = " ".join(name_display) if name_display else f"건축물 {idx+1}"

                st.markdown(f'<div class="bld-header">📌 {name_str}</div>', unsafe_allow_html=True)
