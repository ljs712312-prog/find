...
- 다가구주택의 건축인허가 이력 기반 호(가구)별 전유·공용면적 참고조회
...
지번은 별도의 건축인허가정보 API에서 호별개요와 호별전유공용면적을 관리
PK로 연결해 사용자가 버튼을 누를 때 조회하지만, 이는 인허가 이력 참고값이며
현재 대장 확정값이 아닙니다. 서로 다른 인허가 이력은 합치지 않고, 누락값을 0으로
추정하지 않습니다. 법적 확인은 관할청·세움터 등에서 발급한 원본 대장이
필요합니다.
...
- 집합건물 면적은 전유부 관리 PK로 전유공용면적을 결합하고, `전유`와
  `공용` 행을 각각 합산합니다.
- 다가구 호별 참고면적은 건축인허가의 `관리허가대장PK → 동별개요PK →
  호별개요PK → 호별전유공용면적PK` 관계만 사용합니다. 건축물대장 PK와
  인허가 PK를 직접 연결하거나, 여러 인허가 이력을 합산하지 않습니다.
