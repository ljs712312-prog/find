...
서울·수원 지번을 국토교통부 건축HUB 건축물대장정보 서비스로 조회하는
...
위반건축물은 정부24·세움터 원본 대장 확인 링크와 설정된 경우 VWorld 참고조회를
제공합니다. 전국용 건축HUB API와 현재 배포의 인증 설정을 그대로 사용하므로
서울 전용 API 키는 필요하지 않습니다. 데이터가 없는 지번이나 제공되지 않는
